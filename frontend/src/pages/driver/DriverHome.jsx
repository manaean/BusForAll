...
export default function DriverHome() {
  ...
  useEffect(() => {
    Promise.allSettled([getMyDriverProfile(), getMyAssignment()]).then(async ([p, a]) => {
      if (p.status === 'fulfilled') setProfile(p.value.data);
      if (a.status === 'fulfilled') {
        const assign = a.value.data;
    ...
    getAlerts(true).then(r => setAlerts(r.data)).catch(() => {});
  }, []);
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '0.5rem 1rem', fontSize: '.9rem', fontWeight: 600, color: '#374151', whiteSpace: 'nowrap' }}>
          {formatTime(time)}
        </div>
      </div>
        ...
        <>
          ...
          <div style={{ background: 'linear-gradient(135deg, #0f2a3d 0%, #1a5a7a 100%)', borderRadius: 14, padding: '1.75rem 2rem', marginBottom: '1.5rem' }}>
            ...
            {assignment ? (
              ...
              <p style={{ color: 'rgba(255,255,255,0.65)', margin: 0 }}>No assignment for today.</p>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.5rem', alignItems: 'flex-start' }} className="landing-columns">
            ...
            <div style={{ flex: 1, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <h2 style={{ fontWeight: 700, fontSize: '1rem', color: '#111827', margin: 0 }}>Daily Schedule</h2>
                <Link to="/driver/schedule" style={{ color: '#1a5a7a', fontSize: '.83rem', fontWeight: 600, textDecoration: 'none' }}>Full Schedule</Link>
              </div>
              {schedules.length === 0 ? (
                <p style={{ color: '#9ca3af', fontSize: '.875rem' }}>No schedule data available.</p>
              ) : schedules.slice(0, 3).map(s => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: '#f8fafc', borderRadius: 10, padding: '0.85rem 1rem', marginBottom: '0.6rem' }}>
                  <div style={{ background: '#1a5a7a', borderRadius: 8, padding: '0.45rem 0.65rem', textAlign: 'center', flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: '.9rem', fontWeight: 800, color: '#fff' }}>{s.departureTime?.slice(0, 5)}</div>
                  </div>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '.78rem', color: '#6b7280', marginTop: 2 }}>
                      Arrival: {s.arrivalTime?.slice(0, 5)} &middot; {s.days || 'Daily'}
                    </div>
                  </div>
            ...
            <div style={{ width: 260, flexShrink: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.25rem' }}>
              ...
              {assignment?.Route?.id && (
                <Link to={`/driver/bus/${assignment.Route.id}`} style={{ display: 'block', textAlign: 'center', marginTop: '1rem', padding: '0.6rem', border: '1.5px solid #1a5a7a', borderRadius: 8, color: '#1a5a7a', textDecoration: 'none', fontSize: '.875rem', fontWeight: 600 }}>
                  Track My Bus
                </Link>
              )}
            </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.25rem' }}>
            <h2 style={{ fontWeight: 700, fontSize: '1rem', color: '#111827', marginBottom: '1rem' }}>Active Route Alerts</h2>
            {alerts.length === 0 ? (
              <p style={{ color: '#9ca3af', fontSize: '.875rem' }}>No active alerts.</p>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                {alerts.map(a => (
                  <div key={a.id} style={{ background: a.severity === 'high' ? '#fef2f2' : '#fffbeb', borderRadius: 10, padding: '1rem' }}>
                    <div style={{ fontWeight: 700, fontSize: '.875rem', color: '#111827', marginBottom: '0.35rem' }}>{a.title}</div>
                    <div style={{ fontSize: '.82rem', color: '#6b7280', lineHeight: 1.5 }}>{a.message}</div>
                  </div>
                ))}
              </div>
            )}
