...
const emptyForm = { routeId: '', departureTime: '', arrivalTime: '', days: '' };
...
export default function ManageSchedules() {
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = { ...form, routeId: parseInt(form.routeId) };
  ...
  const startEdit = (s) => {
    setForm({ routeId: s.routeId, departureTime: s.departureTime, arrivalTime: s.arrivalTime, days: s.days || '' });
      ...
      <div style={page}>
        ...
        <div className="table-scroll" style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr><th style={th}>Route</th><th style={th}>Departure</th><th style={th}>Arrival</th><th style={th}>Days</th><th style={th}>Actions</th></tr></thead>
            <tbody>
              {schedules.length === 0 ? <tr><td colSpan={5} style={{ ...td, color: 'var(--text-light)', textAlign: 'center' }}>No schedules found.</td></tr> :
                schedules.map(s => (
                  <tr key={s.id}>
