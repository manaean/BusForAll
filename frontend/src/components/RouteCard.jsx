...
const s = {
  card: { background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: '1rem 1.25rem', cursor: 'pointer', transition: 'box-shadow .2s', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  name: { fontWeight: 600, fontSize: '1rem', color: 'var(--primary)' },
  desc: { color: 'var(--text-light)', fontSize: '.85rem', marginTop: 4 },
  trackBtn: { background: '#E3F2FD', color: 'var(--primary)', border: 'none', fontSize: '.78rem', padding: '4px 10px', borderRadius: 12, fontWeight: 600, cursor: 'pointer' },
...
export default function RouteCard({ route, onFavToggle, isFav, isGuest }) {
  ...
  return (
    <div style={s.card} onMouseEnter={e => e.currentTarget.style.boxShadow='0 4px 12px rgba(0,0,0,.1)'} onMouseLeave={e => e.currentTarget.style.boxShadow='none'}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <button style={s.trackBtn} onClick={() => navigate(`/tracker/${route.id}`)}>Track Bus</button>
